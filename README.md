...
|-----------|------------|--------|
| Agent mesh & FinOps | Python 3.14 | Source in `backend/` |
| Local LLM | Ollama (Llama 3.1) | `http://localhost:11434` |
| Cloud simulation | LocalStack | `http://localhost:4566` |
| Showcase website | React, Vite, TypeScript, Tailwind CSS v4, Framer Motion, Lucide React | Source in `web/` — interactive simulation, metrics, architecture diagram |

**Node.js** (current LTS recommended) is required only for the `web/` frontend (`npm install`, `npm run dev`).
...
├── backend/                    # Python agent mesh & FinOps logic
...
│   ├── budget_guard.py         # Cost-prediction interceptor & Dynamic Router
│   ├── demo.py                 # Frugal dynamic routing demonstration
...
│   └── pyrightconfig.json      # Pyright settings for this package
├── web/                        # React showcase (Cyber-FinOps marketing / demo UI)
│   ├── src/
│   │   ├── components/       # Hero, simulation, metrics, architecture, evidence
│   │   ├── hooks/
│   │   ├── App.tsx
│   │   └── main.tsx
│   ├── package.json
...
#### `backend/brain.py` - The Voice Box
...
#### `backend/budget_guard.py` - The Proxy Interceptor
...
#### `backend/accountant_agent.py` - The Fiscal Authority
...
#### `backend/bridge.py` - Phase 1 Milestone
...
#### `backend/researcher.py` - Researcher Agent
...
#### `backend/writer.py` - Writer Agent
...
Then open the URL printed in the terminal (typically `http://localhost:5173`). Use `npm run build` for a production bundle in `web/dist`.

The site includes a **Cyber-FinOps** themed hero, an **interactive simulation** (standard vs budget-aware modes with a live budget gauge and execution log), **governance metric** cards with animated values, an **architecture mesh** diagram, and a **proof-of-execution** ledger block. Point the IEEE paper CTA to your publication URL in `web/src/components/PaperSection.tsx` when ready.
